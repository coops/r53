...
def fetch_config(zone, conn):
  ...
  while more_to_fetch == True:
    more_to_fetch = False
    getstr = '/2011-05-05/hostedzone/%s/rrset' % zone
    ...
    root = etree.getroot()
    truncated = root.find('{https://route53.amazonaws.com/doc/2011-05-05/}IsTruncated')
    if truncated is not None and truncated.text == 'true':
      more_to_fetch = True
      next_name = root.find('{https://route53.amazonaws.com/doc/2011-05-05/}NextRecordName').text
      next_type = root.find('{https://route53.amazonaws.com/doc/2011-05-05/}NextRecordType').text
      try:
        next_identifier = root.find('{https://route53.amazonaws.com/doc/2011-05-05/}NextRecordIdentifier').text
...
def merge_config(cfg_chunks):
  ...
  root = lxml.etree.XML('<ResourceRecordSets xmlns="https://route53.amazonaws.com/doc/2011-05-05/"></ResourceRecordSets>')
  for chunk in cfg_chunks:
    for rrset in chunk.iterfind('.//{https://route53.amazonaws.com/doc/2011-05-05/}ResourceRecordSet'):
      root.append(rrset)
  return root
...
def generate_changeset(old, new):
  ...
  if '{https://route53.amazonaws.com/doc/2011-05-05/}ResourceRecordSets' not in (old.tag, new.tag):
    log.error('both configs must be ResourceRecordSets tags. old: %s, new: %s' % (old.tag, new.tag))
    raise InvalidArgumentException()

  root = lxml.etree.XML('<ChangeResourceRecordSetsRequest xmlns="https://route53.amazonaws.com/doc/2011-05-05/"><ChangeBatch><Comment>Generated by %s for %s@%s at %s.</Comment><Changes/></ChangeBatch></ChangeResourceRecordSetsRequest>' % (
      __file__, os.environ['USER'], os.environ['HOSTNAME'], time.strftime('%Y-%m-%d %H:%M:%S')))
  changesroot = root.find('.//{https://route53.amazonaws.com/doc/2011-05-05/}Changes')
  ...
  for rrs in old:
    ...
    if rrsst not in newset:
      log.debug("REMOVED:")
      log.debug(rrsst)
      change = lxml.etree.XML('<Change xmlns="https://route53.amazonaws.com/doc/2011-05-05/"><Action>DELETE</Action></Change>')
      change.append(rrs)
  ...
  for rrs in new:
    ...
    if rrsst not in oldset:
      log.debug("ADDED:")
      log.debug(rrsst)
      change = lxml.etree.XML('<Change xmlns="https://route53.amazonaws.com/doc/2011-05-05/"><Action>CREATE</Action></Change>')
      change.append(rrs)
      changesroot.append(change)
  return root
...
def validate_changeset(changeset):
  ...
  errors = []
  changes = changeset.findall('.//{https://route53.amazonaws.com/doc/2011-05-05/}Change')
  ...
  rrs = changeset.findall('.//{https://route53.amazonaws.com/doc/2011-05-05/}ResourceRecord')
  num_rrs = len(rrs)
  if num_rrs > 1000:
    errors.append('changeset has %d ResourceRecord elements: max is 1000' % num_rrs)
  values = changeset.findall('.//{https://route53.amazonaws.com/doc/2011-05-05/}Value')
  num_chars = 0
  for value in values:
